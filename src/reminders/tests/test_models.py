...
class ReminderTest(TestCase):
    ...
    def test_reminder_fail_creation(self):
        with self.assertRaises(ValidationError):
            obj = self.create_reminder(
                self.MESSAGE, self.DATE, self.NEGATIVE_TIME, self.EMAIL)
        logger.info("Fail test passed")

    def test_reminder_success_creation(self):
        obj = self.create_reminder(
            self.MESSAGE, self.DATE, self.POSITIVE_TIME, self.EMAIL, '')
    ...
    def test_reminder_email_phone_number_both_present(self):
        obj = self.create_reminder(
            self.MESSAGE, self.DATE, self.POSITIVE_TIME, self.EMAIL, self.PHONE_NUMBER)
        self.assertTrue(obj.email is not None)
